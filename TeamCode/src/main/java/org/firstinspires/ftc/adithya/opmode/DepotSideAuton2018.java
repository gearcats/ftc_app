package org.firstinspires.ftc.adithya.opmode;
...
public class DepotSideAuton2018 extends LinearOpMode {
	...
	private static final String VUFORIA_KEY =
			"AQL3GSz/////AAABmU0iuBTYZUZyqsDIbBriyR5+fo3kTV9e8dpVguROyqV8Cfa//LkjRFXUIK22aroE9+JL+K6fSkFKoR7WmLw6Rh9"
					+ "+/726CK1FGzcdaZ62wVJlP2Ksex6H2FLpJevxHcfBLTsywxWuLYDGwxCywCmgb9zrzyjGWj8z9Qd" +
					"+0wkg9GQolVdEaZVd1p8xNNHicFRyf5X5u1pna8F/GIm9xXBTtmURyo/msigPolDTqKLBIN+60S609XoVM4dE" +
					"/d7NYNGILqj6WVZfPqRy+NyWMfuc3os1RiUWWxK7UbVE7yDrUJeRS5fy6ikVZJaSdn8+zkWDpPayrDVncwf9" +
					"+kTL4aExzdjQwSbn32X5OWXWOOpoB3no";
	...
	private final BNO055IMU.Parameters parameters = new BNO055IMU.Parameters();//the parameters we will later set to
	// inizialize the imu
	...
	public void getImuDataAndOutputToTelemtry() {
		...
		telemetry.addLine().addData("grvty", new Func<String>() {
			...
			@Override
			public String value() {
				mag = String.format(Locale.getDefault(), "%.3f", Math.sqrt(gravity.xAccel * gravity.xAccel + gravity
						.yAccel * gravity.yAccel + gravity.zAccel * gravity.zAccel));
				return String.format(Locale.getDefault(), "%.3f", Math.sqrt(gravity.xAccel * gravity.xAccel + gravity
						.yAccel * gravity.yAccel + gravity.zAccel * gravity.zAccel));
			}
		});
	...
	public void persiseMove(double cm, double motorPower) {
		...
		final double RATIO                = 1440.0 / WHEEL_CICUM;//28.647889

		final int goalLeft  = -1 * (int) ((cm * RATIO) + (double) left.getCurrentPosition());//859
		final int goalRight = (int) ((cm * RATIO) + (double) right.getCurrentPosition());//-859
		...
		if (cm > 0) {
			while (left.getCurrentPosition() > goalLeft && right.getCurrentPosition() < goalRight) {
				left.setPower(-motorPower);//left is mounted backwards and we want to go forwards
			...
			telemetry.addData("Bozo0", "why would you move 0 cm.");
			telemetry.update();
		}
		left.setPower(0);
		right.setPower(0);
	...
	public void runOpMode() {
		...
		if (opModeIsActive()) {
			...
			telemetry.addData("moving", "stuff");
			telemetry.update();
            ...
            persiseTurn(179.9, 0.6);
            getImuDataAndOutputToTelemtry();

            ElapsedTime vuforiaSearchTime = new ElapsedTime();
            double secondsSinceStart = vuforiaSearchTime.seconds();
            while (goldPosition != "Left" && goldPosition != "Right" && goldPosition != "Center"  &&
            secondsSinceStart > 4.0)
            {
                checkForMinerals();
                secondsSinceStart = vuforiaSearchTime.seconds();
            }

            if (goldPosition == "left")
            {
                persiseTurn(38.0, 0.4);
                persiseMove(67.3, 0.8);

                persiseTurn(58, 0.7);
                persiseMove(91.4, 0.8);

                persiseTurn(28, 0.7);
            }
            else if (goldPosition == "Right")
            {
                persiseTurn(35.0, 0.7);
                persiseMove(61.0, 0.8);

                persiseTurn(66.0, 0.7);
                persiseMove(81.3, 0.8);

                persiseTurn(16.0, 0.7);
            }
            else if (goldPosition == "Center")
            {
                persiseMove(56.0, 0.8);

                persiseTurn(-10, 0.7);
                persiseMove(99.0, 0.8);
            }
            else
            {
                //go right
            ...
            //realease the
            persiseTurn(-48.0, 0.7);
            persiseMove(152.4, 0.8);*/
		}
		/////////////////////////////////////////////////////////////////////////////////////////////////////////
		if (tfod != null) {
			tfod.shutdown();
