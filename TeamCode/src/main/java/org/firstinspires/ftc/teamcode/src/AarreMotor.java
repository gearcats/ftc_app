...
public class AarreMotor implements AarreMotorInterface {
	...
	double secondsTimeout) throws NoSuchMethodException {
		...
		log.debug(String.format("Motor - Ramp to encoder ticks(3), target power UP: %f" + powerVector.asDouble()));
		rampToPower(powerVector, ticksToSpeedUp, secondsTimeout);

		log.debug(String.format("Motor - Ramp to encoder ticks(3), target power DOWN: %f", 0.0));
	...
	private void speedUpToPower(final AarrePowerVector powerVectorRequested, final AarrePositiveInteger ticksToMove,
		...
		log.debug(String.format("Target power: %f", powerVectorRequested));
		log.debug(String.format("Target ticks: %d", ticksToMove));
		...
		while (!isSpeedUpToEncoderTicksDone(ticksToMove, secondsTimeout, secondsRunning, ticksMoved)) {
			...
			log.debug(String.format("Motor::speedUpToPower(3) - New power: %f", powerVectorNew));
	...
	final void runByRevolutions(final AarrePowerVector proportionMotorPower, final double targetNumberOfRevolutions,
		...
		log.debug(String.format("Proportion motor power: %f", proportionMotorPower.asDouble()));
		...
		log.debug(String.format("Number of ticks to run (AarrePositiveInteger): {}", numberOfTicksToRun.doubleValue()));
